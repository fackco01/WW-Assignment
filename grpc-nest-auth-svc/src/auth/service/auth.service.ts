...
import { DeleteRequest, DeleteResponse, LoginRequest, LoginResponse, RegisterResponse, ValidateRequest, ValidateResponse } from "../auth.pb";
...
export class AuthService implements OnModuleInit {
    ...
    public async deleteAuth(data: DeleteRequest): Promise<DeleteResponse> {
        const auth: Auth = await this.repository.findOneBy({ id: data.id });
        await this.repository.remove(auth);
        return {
            status: HttpStatus.OK,
        };
